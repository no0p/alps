...
void process_columns(void) {
  ...
  " AND NOT EXISTS ("
  " SELECT * FROM pg_index INNER JOIN pg_class ON pg_index.indrelid = pg_class.oid WHERE pg_attribute.attrelid = pg_index.indrelid AND indisprimary is true AND pg_index.indkey::text = pg_attribute.attnum::text "
  " )"
  " ORDER BY relname),"
